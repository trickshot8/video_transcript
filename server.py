...
def _process(job_id: str, url: str):
    ...
    try:
        ...
        msg = f"{badge} {result.source_label} · {result.segment_count}条\n《{result.title}》"
        # message 同时放到顶层，方便 iOS 快捷指令用「是否有值」判断是否处理完
        # text 同时放顶层，方便快捷指令直接取来复制到剪贴板
        _set(job_id, status="done", message=msg, text=result.text, result={
            "title": result.title,
            "bvid": result.bvid,
            "level": result.level,
