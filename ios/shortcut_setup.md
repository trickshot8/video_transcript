...
照着下面这套搭，先有个全貌再看细节会更容易跟上：

```text
分享链接
  │
  ▼
① 提交任务  POST /jobs  →  拿到 job_id
  │
  ▼
② 轮询      GET /jobs/<id>，每隔几秒查一次，直到有 message
  │
  ▼
③ 复制全文  把 text 存进剪贴板
  │
  ▼
④ 决策菜单  弹出摘要置顶的菜单：删除 / 保留 / 归类
                                          │
                                          ▼
                              ⑤ 归类子流程：选/建文件夹 → 移动文件
```

**为什么提交后要"轮询"而不是直接等结果**：服务端处理完才有结果，轮询是先拿一张
"取件号"`job_id`、再隔几秒问一次"好了没"，这样无论快慢都不会让快捷指令卡死等超时。
关键技巧：**拿到结果就存进 `RESULT` 变量、后续循环空转跳过**，这样第④步的菜单逻辑
能放在循环外面写，不用塞进循环里层层嵌套（这是最容易卡住的地方，务必照着下面的结构搭）。
...
1. **「获取URL内容」里有两个"URL"，别填混了**：最顶上的 URL 框是**请求发到哪**（填
   `BASE/jobs` 这个服务器地址），JSON 请求体里的 `url` 字段才是**要处理的视频链接**
   （填快捷指令输入）。两个都叫 url，但完全不是一回事——填反了请求会直接失败。

2. **加「如果(If)」时，条件默认变量经常是 `File Size`，不是你想要的字段**：Shortcuts
   插入 If 时会自动塞一个默认变量，容易顺手就用了它。务必手动点开换成正确的变量
   （比如 `message`），否则条件永远判不对、循环会一直空转到超时。

3. **URL 框里别把"字面文字 + 变量"拼在一起**，否则可能报
   `couldn't convert from Rich Text to URL`：例如轮询时想拼 `BASE/jobs/` + `JOB`，
   别直接在 URL 框里这样拼——先用一个**文本（Text）**动作把它们拼成一整条纯文本，
   再把这个 Text 变量整个放进 URL 框（URL 框里只放这一个变量）。
