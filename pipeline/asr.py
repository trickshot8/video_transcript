...
def download_audio(bvid: str, page: Optional[int] = None) -> Path:
    """下载视频音轨为 16k 单声道 wav，返回文件路径。"""
    import yt_dlp
    ...
    out_base = config.TMP_DIR / f"{bvid}_{page or 1}_{int(time.time())}"
    opts = {
...
def transcribe_local(info: VideoInfo, page: Optional[int] = None) -> SubtitleResult:
    audio = download_audio(info.bvid, page)
    try:
        model = _get_model()
